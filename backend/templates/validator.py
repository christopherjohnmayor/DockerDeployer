...
def validate_variables(template_data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    for var_name, var_config in variables.items():
        ...
        if not re.match(r'^[A-Z][A-Z0-9_]*$', var_name):
            return False, f"Invalid variable name format: {var_name}. Must be uppercase with underscores."
        ...
        if options:
            if not isinstance(options, list) or not options:
                return False, f"Options for variable {var_name} must be a non-empty list"
            
            default = var_config.get("default")
            if default is not None and str(default) not in [str(opt) for opt in options]:
...
def validate_services(template_data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    for service_name, service_config in services.items():
        ...
        if not re.match(r'^[a-z][a-z0-9_\-]*$', service_name):
            return False, f"Invalid service name: {service_name}. Use lowercase letters, numbers, hyphens, and underscores."
        
        # Check service configuration
        if not isinstance(service_config, dict):
            return False, f"Service configuration for {service_name} must be a dictionary"
        
        # Check if service has image or build
        if "image" not in service_config and "build" not in service_config:
            return False, f"Service {service_name} must have either 'image' or 'build' defined"
            ...
            if isinstance(value, str):
                ...
                for var_ref in var_refs:
                    if var_ref not in template_data.get("variables", {}):
                        return False, f"Service {service_name} references undefined variable: {var_ref}"
...
def validate_files(template_data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
    ...
    for file_entry in files:
        ...
        if ".." in path or path.startswith("/"):
            return False, f"Invalid file path: {path}. Path cannot contain '..' or start with '/'"
    ...
    for var_name, var_config in variables.items():
        if var_name not in variable_values:
            ...
            if options and str(value) not in [str(opt) for opt in options]:
                return False, f"Value for {var_name} must be one of: {', '.join(map(str, options))}", {}
