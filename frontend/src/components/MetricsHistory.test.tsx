...
describe('MetricsHistory', () => {
  ...
  it('displays error state', () => {
    ...
    expect(screen.getByText(`Failed to load historical metrics: ${errorMessage}`)).toBeInTheDocument();
  });
  ...
  it('handles API errors gracefully', async () => {
    ...
    await waitFor(() => {
      expect(consoleSpy).toHaveBeenCalledWith('Error fetching historical metrics:', expect.any(Error));
    });
  ...
  it('handles empty metrics response', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText(/Showing \d+ data points/)).not.toBeInTheDocument();
    });
  });
