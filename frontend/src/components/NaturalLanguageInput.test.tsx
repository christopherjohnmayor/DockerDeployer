...
describe('NaturalLanguageInput Component', () => {
  ...
  test('sends command and displays response', async () => {
    ...
    expect(mockedAxios.post).toHaveBeenCalledWith('/api/nlp/parse', {
  ...
  test('shows and hides command history', async () => {
    ...
    localStorage.setItem('commandHistory', JSON.stringify(['List containers', 'Deploy WordPress']));
  ...
  test('shows and hides suggestions', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('Deploy a WordPress stack')).not.toBeInTheDocument();
    });
  });
  ...
  test('adds command to history when sent', async () => {
    ...
    // Check if the command was added to history
    expect(screen.getByText('List all containers')).toBeInTheDocument();
  ...
  test('fills input when clicking on a suggestion', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText('Start all containers')).not.toBeInTheDocument();
    });
  });
});
