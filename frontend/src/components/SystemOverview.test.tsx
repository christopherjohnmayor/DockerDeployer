...
describe('SystemOverview', () => {
  ...
  it('displays error state', () => {
    ...
    expect(screen.getByText('System Overview')).toBeInTheDocument();
    expect(screen.getByText(`Failed to load system metrics: ${errorMessage}`)).toBeInTheDocument();
  });
  ...
  it('handles API errors gracefully', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('Error fetching system metrics:', expect.any(Error));
    consoleSpy.mockRestore();
  });

  it('handles error response from API', async () => {
    const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
    mockUseApiCall.execute.mockResolvedValue({ error: 'Docker daemon unavailable' });
